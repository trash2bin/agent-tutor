...
|---|---|---|---|
| `data-service` | **Go** (chi, modernc/sqlite) | 8084 | Доступ к БД университета. Config-driven: схема в JSON, SQL генерируется runtime. Сценарии БД (`testdata/scenarios/`) — фабрика тестовых баз через `--materialize`/`.scripts/dev.sh db` |
