...
from rag.models import Document, ChunkDict
...
def test_delete_document_record(repo):
    ...
    assert cursor.execute("SELECT id FROM document_chunks WHERE document_id=?", (doc_id,)).fetchone() is not None
    ...
    assert cursor.execute("SELECT id FROM document_chunks WHERE document_id=?", (doc_id,)).fetchone() is None
