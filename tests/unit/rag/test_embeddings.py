...
def test_encode_batched_logic():
    ...
    mock_model.encode.side_effect = lambda texts, **kwargs: [np.random.rand(384).tolist() for _ in texts]
...
def test_model_load_failure():
    ...
    with patch("sentence_transformers.SentenceTransformer", side_effect=Exception("Download error")):
