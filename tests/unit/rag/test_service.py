...
async def mock_state():
    ...
    with patch.object(state, "get_pipeline") as mock_pipe, \
...
async def test_list_documents_success(mock_state):
    ...
    mock_doc = Document(
        id="doc1", title="Title 1", source_path="p1.txt",
        mime_type="text/plain", discipline_id="d1", created_at="now"
    )
    pipeline.list_documents.return_value = [mock_doc]
    
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        # Тест POST версии
        response = await ac.post("/documents/list", json={"discipline_id": "d1", "limit": 10})
        assert response.status_code == 200
        data = response.json()
        assert data["count"] == 1
...
async def test_import_document_success(mock_state):
    ...
    mock_doc = Document(
        id="doc_new", title="New Doc", source_path="new.txt",
        mime_type="text/plain", discipline_id="d1", created_at="now"
    )
...
async def test_delete_document_success(mock_state):
    ...
    mock_repo.find_document_for_delete.return_value = {
        "id": "doc_del", "title": "To Delete", "source_path": "del.txt"
    }
...
async def test_search_success(mock_state):
    ...
    mock_result = RagSearchResult(
        document_id="doc1", document_title="T1", source_path="p1",
        discipline_id="d1", chunk_id="c1", chunk_index=0,
        page=1, score=0.95, content="Relevant content"
    )
...
async def test_context_success(mock_state):
    ...
    # Мокаем возвращаемый объект с методом model_dump
    mock_context = MagicMock()
    mock_context.model_dump.return_value = {
        "context": "The actual context text",
        "sources": [{"id": "doc1", "title": "T1"}]
    }
