...
Используй `uv run ...` для запуска и отладки частей приложения.
...
RAG выделен в **отдельный HTTP-сервис** (`rag/service.py` на Starlette, порт 8082) с HTTP-клиентом (`rag/client.py`) для вызовов из MCP и других компонентов.
