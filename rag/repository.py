...
class DocumentRepository:
    ...
    def db_lock(self) -> threading.RLock:
        """Reentrant lock для thread-safe доступа к self.conn.

        Используется для длинных транзакций из вызывающего кода (pipeline),
        где нужно держать блокировку на несколько операций репозитория.
        """
        return self._lock
