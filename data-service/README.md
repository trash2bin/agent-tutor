...
### Strict Multi-tenancy (Фаза 3.7+ ✅)

Data-service работает в режиме **строгой мультитеннантности**. Это означает, что в системе полностью отсутствует понятие «дефолтного» тенанта. Любой запрос к данным ДОЛЖЕН содержать идентификатор тенанта.

**Как это работает:**
1. **Обязательный идентификатор**: Сервис извлекает тенант из заголовка `X-Tenant-ID` или query-параметра `?tenant=`.
2. **Никаких фоллбэков**: Если идентификатор отсутствует или тенант не найден в `TenantStore`, сервис возвращает `404 Not Found (tenant_not_found)`.
3. **Динамическое управление**: Тенанты добавляются, обновляются и удаляются через Admin API (`/admin/tenants`) без перезагрузки сервиса.
4. **Изоляция**: Каждый тенант имеет свой собственный пул соединений с БД, конфигурацию и изолированный HTTP-роутер.

**Маршрутизация и X-Tenant-ID:**
- Запрос $\rightarrow$ `TenantStore.ServeHTTP` $\rightarrow$ поиск `TenantInstance` $\rightarrow$ вызов `instance.Router.ServeHTTP`.
- Без `X-Tenant-ID` $\rightarrow$ `404 Not Found`.

**API для тестов:**
- `TenantStore.RegisterTenantInstance(inst)` — регистрирует pre-built `TenantInstance` без открытия нового соединения с БД. Критично для тестов: `AddTenant()` открывает свежий `:memory:` SQLite (теряя pre-seeded данные), а `RegisterTenantInstance` регистрирует уже готовый инстанс.
- `HealthCheck` обрабатывает `nil Conn` (тестовые инстансы): если `Conn == nil`, тенант считается healthy.

**Формат `/health`:**
- Multi-tenant режим: `{"status":"ok"}` без поля `"db"` (агрегированный статус всех тенантов).
- Single-tenant (legacy): `{"status":"ok","db":"ok"}`.
...
# Управление сценариями через agent-db CLI
uv run agent-db scenario list                              # список сценариев
uv run agent-db materialize big-testseed --force           # пересоздать большой сценарий
uv run agent-db e2e --tenants default                      # полный E2E: materialize → register → proxy → chat
uv run agent-db e2e-full                                   # все E2E-уровни (data + mcp + chat)
...
```bash
# Все Go-тесты — data-service + mcp-gateway (274 теста в 14 пакетах)
go test ./data-service/... ./mcp-gateway/... -count=1

# Только unit-тесты runtime/configgen/datasource
cd data-service && go test ./internal/configgen/... ./internal/runtime/... ./internal/datasource/...

# Сценарии (in-memory SQLite)
cd data-service && go test ./internal/server/... -run TestScenario -v
